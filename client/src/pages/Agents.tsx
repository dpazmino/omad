        ...
        <div className="p-8 max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4].map(i => (
                <Card key={i} className="bg-white border border-border p-6 h-48 animate-pulse rounded-xl" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {agents.map((agent) => (
                <Card key={agent.id} className="bg-white border border-border p-6 flex flex-col gap-4 rounded-xl shadow-sm hover:shadow-md hover:border-primary/20 transition-all" data-testid={`card-agent-${agent.id}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-primary/8 border border-primary/10 flex items-center justify-center text-xl">
                        {agent.icon}
                      </div>
                      <div>
                    ...
                    <button className="text-muted-foreground hover:text-foreground transition-colors p-2 hover:bg-muted rounded-md" data-testid={`button-agent-settings-${agent.id}`}>
                      <Settings2 size={16} />
                    </button>
                  </div>
                  ...
                  {agent.menuCommands && (agent.menuCommands as any[]).length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {(agent.menuCommands as any[]).map((cmd: any) => (
                        <span key={cmd.trigger} className="px-2 py-0.5 rounded bg-muted text-[10px] text-muted-foreground font-mono border border-border">
                          {cmd.trigger}
                        </span>
                      ))}
                    </div>
